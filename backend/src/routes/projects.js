...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const project = await Project.create({ name, description, members: [req.user._id, ...(members || [])], createdBy: req.user._id });
...
router.delete('/:id/members/:memberId', auth, permit('Admin'), async (req, res) => {
  try {
    const project = await Project.findById(req.params.id);
    if (!project) return res.status(404).json({ message: 'Project not found' });
